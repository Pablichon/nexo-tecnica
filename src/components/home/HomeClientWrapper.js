...
export default function HomeClientWrapper({ initialOffers = [] }) {
  const [selectedOffer, setSelectedOffer] = useState(null);
  const featuredOffers = initialOffers.filter(o => o.destacada);

  const handleCategoryClick = (categoryName) => {
    trackEvent('Filtro Categoria', { categoria: categoryName });
  };
